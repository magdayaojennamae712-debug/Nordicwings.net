...
function renderAffiliateResults(orig, dest, date, adults, children, infants) {
  ...
  const tripUrl    = `https://www.trip.com/flights/show?dcity=${orig}&acity=${dest}&ddate=${tripDate}&adult=${pax}&child=0&infant=0&tripType=OW&class=Economy&${TC}`;
...
function renderFlightCards(flights) {
  ...
  const tripUrl = (kiwiOrigin && kiwiDest && kiwiDateTrip)
    ? `https://www.trip.com/flights/show?dcity=${kiwiOrigin}&acity=${kiwiDest}&ddate=${kiwiDateTrip}&adult=${kiwiPass}&child=0&infant=0&tripType=OW&class=Economy&Allianceid=8098413&SID=306552835&trip_sub1=flights&trip_sub3=D16144585`
...
function openPartnerLink(agencyName) {
  ...
  const links = {
    ...
    'Trip.com':   `https://www.trip.com/flights/show?dcity=${orig}&acity=${dest}&ddate=${date.replace(/-/g,'')}&adult=${pass}&child=0&infant=0&tripType=OW&class=Economy&${TC}`,
  };
