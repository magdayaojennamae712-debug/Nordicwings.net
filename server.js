...
let sabreToken       = null;
let sabreTokenExpiry = 0;

async function getSabreToken() {
  if (sabreToken && Date.now() < sabreTokenExpiry) return sabreToken;
  if (!SABRE_CLIENT_ID || !SABRE_CLIENT_SECRET) return null;

  try {
    // Sabre uses nested base64 encoding
    const clientId     = Buffer.from(SABRE_CLIENT_ID).toString('base64');
    const clientSecret = Buffer.from(SABRE_CLIENT_SECRET).toString('base64');
    const auth         = Buffer.from(`${clientId}:${clientSecret}`).toString('base64');

    const res = await fetch(`${SABRE_BASE_URL}/v2/auth/token`, {
      method: 'POST',
      headers: {
    ...
    sabreToken       = data.access_token;
    sabreTokenExpiry = Date.now() + (data.expires_in - 60) * 1000;
...
async function searchSabreFlights(orig, dest, date, adults) {
  const token = await getSabreToken();
  if (!token) return null;

  const depDateTime = `${date}T00:00:00`;
  ...
  const body = {
    OTA_AirLowFareSearchRQ: {
      ...
      OriginDestinationInformation: [{
        ...
        OriginLocation:      { LocationCode: orig },
      ...
      TravelPreferences: {
        ...
        TPA_Extensions: { TripType: { Value: 'ONE_WAY' } }
      },
      TravelerInfoSummary: {
  ...
  try {
    const res = await fetch(`${SABRE_BASE_URL}/v3/offers/shop`, {
      method:  'POST',
      headers: {
    ...
    const data = await res.json();
    const itins = data?.groupedItineraryResponse?.itineraryGroups?.[0]?.itineraries
                || data?.OTA_AirLowFareSearchRS?.PricedItineraries?.PricedItinerary
                || [];

    if (!itins.length) return null;

    // Map Sabre results to our flight format
    const flights = [];
    for (let i = 0; i < Math.min(itins.length, 8); i++) {
      try {
        const it  = itins[i];
        // Handle both response formats
        const leg = it.legs?.[0] || it;
        const airItinerary = it.AirItinerary || it;
        const priceInfo    = it.AirItineraryPricingInfo?.[0] || it.pricingInformation?.[0];
        const price        = priceInfo?.ItinTotalFare?.TotalFare?.Amount
                          || priceInfo?.fare?.totalFare?.totalPrice
                          || 0;

        const originDestOptions = airItinerary?.OriginDestinationOptions?.OriginDestinationOption || [];
        const segments = (originDestOptions[0]?.FlightSegment || []).map(seg => ({
          departure: { iataCode: seg.DepartureAirport?.LocationCode || orig, at: seg.DepartureDateTime || date },
          arrival:   { iataCode: seg.ArrivalAirport?.LocationCode   || dest, at: seg.ArrivalDateTime   || date },
          carrierCode: seg.MarketingAirline?.Code || 'XX',
          number:      seg.FlightNumber || String(i * 13 + 100),
          duration:    `PT${Math.floor((seg.ElapsedTime || 120) / 60)}H${(seg.ElapsedTime || 120) % 60}M`
        }));

        if (!segments.length) continue;

        const totalMins = segments.reduce((sum, s) => {
          const m = s.duration.match(/PT(\d+)H(\d+)M/);
          return sum + (m ? parseInt(m[1]) * 60 + parseInt(m[2]) : 120);
        }, 0);

        flights.push({
          id: `sabre-${i}`,
          price: {
            grandTotal: (price * adults).toFixed(2),
            currency:   'EUR',
        ...
        console.error('Sabre map error:', mapErr.message);
      }
    }
...
app.get('/api/flights/search', searchLimiter, async (req, res) => {
  ...
  // ── Try Sabre first (best real flight data) ──────────────────
  if (SABRE_CLIENT_ID && SABRE_CLIENT_SECRET) {
    console.log('Trying Sabre API...');
    const sabreFlights = await searchSabreFlights(cleanOrigin, cleanDest, cleanDate, cleanAdults);
    if (sabreFlights && sabreFlights.length) {
      console.log(`Sabre returned ${sabreFlights.length} real flights!`);
      return res.json(sabreFlights);
    }
    console.log('Sabre returned no results — trying Sky Scrapper...');
  }
